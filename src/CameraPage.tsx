...
import { sendPictureToServer, takePhoto } from './handlers/sendPicture';
...
setTimeout(()=>{
   sendInterval.current = setInterval(async () => {
    if (camera) {
     takePhoto(camera,onMediaCaptured,supportsFlash ? flash : 'off');
    ...
    <View style={styles.container}>
      ...
      <View style={styles.rightButtonRow}>
        {/* {supportsCameraFlipping && (
          <PressableOpacity activeOpacity={0.8} style={styles.button} onPress={onFlipCameraPressed} disabledOpacity={0.4}>
            <IonIcon name="camera-reverse" color="white" size={24} />
          </PressableOpacity>
        )} */}
        {supportsFlash && (
          <PressableOpacity activeOpacity={0.8}  style={styles.button} onPress={onFlashPressed} disabledOpacity={0.4}>
            <IonIcon name={flash === 'on' ? 'flash' : 'flash-off'} color="white" size={24} />
          </PressableOpacity>
        )}
        {supports60Fps && (
          <PressableOpacity activeOpacity={0.8} style={styles.button} onPress={() => setIs60Fps(!is60Fps)}>
        ...
        {supportsHdr && (
          <PressableOpacity activeOpacity={0.8} style={styles.button} onPress={() => setEnableHdr((h) => !h)}>
            <MaterialIcon name={enableHdr ? 'hdr' : 'hdr-off'} color="white" size={24} />
          </PressableOpacity>
        )}
        {canToggleNightMode && (
          <PressableOpacity activeOpacity={0.8} style={styles.button} onPress={() => setEnableNightMode(!enableNightMode)} disabledOpacity={0.4}>
